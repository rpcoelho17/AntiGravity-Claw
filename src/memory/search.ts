...
export async function search(
    ...
    try {
        ...
        // Run vector search
        vecResults = vectorSearch(queryBuffer, options.channel, options.collections, topK, options.beforeId);
    } catch (err: any) {
        if (err?.status === 429) {
    ...
    // Run BM25 search
    const bm25Results = bm25Search(query, options.channel, options.collections, topK, options.beforeId);

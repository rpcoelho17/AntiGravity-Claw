...
// ── Migration: Upgrade to 1024d (BGE-M3) ───────────────────────────
try {
    // Check if current embeddings are 768d (legacy)
    const testRow = db.prepare("SELECT embedding FROM memory_embeddings LIMIT 1").get();
    if (testRow) {
        const buf = (testRow as any).embedding;
        if (buf.length === 768 * 4) { // Float32 is 4 bytes
            console.log("⚠️ Legacy 768d embeddings detected. Upgrading to 1024d (BGE-M3)...");
            db.exec(`
                DROP TABLE IF EXISTS memory_embeddings;
                CREATE VIRTUAL TABLE memory_embeddings USING vec0(embedding FLOAT[1024]);
                -- Mark all memory records as needing re-embedding if we had conversation memory
                -- For now, we clear them to ensure consistency
                DELETE FROM memory;
...
console.log(`🧠 Memory DB initialized at ${DB_PATH} (Dimension: 1024)`);
